...
namespace GraphicsEvaluatePlatform.Repository
{
    public abstract class DbHelperSQL
    {
...
#if UNITTEST
        public static string connectionString = @"server=192.168.1.20\NEWSERVERSQL2008;uid=sa;pwd=taitan;database=GraphicsEvaluatePlatform";
#else
            ...
            using (SqlConnection conn = new SqlConnection(connectionString))
            {
                ...
                using (SqlTransaction trans = conn.BeginTransaction())
                {
                    ...
                    catch (Exception ex)
                    {
                        ...
                        trans.Rollback();
                        GraphicsEvaluatePlatform.Infrastructure.Logging.Logger.GetLogger("DbHelperSqL事务").Error("Message:" + ex.Message + "Inner:" + ex.InnerException);
                        result = false;
                        throw;
                    }
        ...
        /// <returns></returns>
        public static DataSet GetDataSetListBySp(string sqlSelect, string sqlTable, int startIndex, int endIndex, string order)
        {
            return ExecuteSqlGetDataSet(CommandType.StoredProcedure, "sp_Page",
                new SqlParameter("@sqlSelect", sqlSelect),
                new SqlParameter("@sqlTable", sqlTable),
                new SqlParameter("@orderByName", order),
        ...
        public static bool SqlBulkCopyByDatatable(string TableName, DataTable dt)
        {
            ...
            using (SqlConnection conn = new SqlConnection(connString))
            {
                using (SqlBulkCopy sqlbulkcopy = new SqlBulkCopy(connectionString, SqlBulkCopyOptions.UseInternalTransaction | SqlBulkCopyOptions.KeepIdentity | SqlBulkCopyOptions.KeepNulls))
                {
