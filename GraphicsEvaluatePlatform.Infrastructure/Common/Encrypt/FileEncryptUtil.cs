using GraphicsEvaluatePlatform.Infrastructure.Common.Files;
using System;
...
namespace GraphicsEvaluatePlatform.Infrastructure.Encrypt
{
    ...
    {
        const string FileType = "TT.Soft.FileEncrypt";//文件类型 禁止修改长度（19位）
        ...
        public static int Encrypt(string srcFile, string dstFile, string password, bool overwrite = true)
        {
            ...
            string fmtPwd = AesUtil.FmtPassword(password);
            string pwdMd5 = EncryptPasswordFactory.GetEncipher(EncryptType.MD5).EncryptFor(EncryptPasswordFactory.GetEncipher(EncryptType.MD5).EncryptFor(fmtPwd));

            string md5 = FileUtil.GetMD5(srcFile);
                    ...
                    try
                    {
                        ...
                        List<byte[]> headdata = new List<byte[]>()
                            {
                                ...
                                Encoding.Default.GetBytes(AesUtil.Encrypt(fmtPwd,AesUtil.DefaultPassword)),
                        ...
                        while ((readCount = fsRead.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            if (readCount != buffer.Length)
                            {
                                ...
                                byte[] enbyte = AesUtil.Encrypt(temp, fmtPwd);
                                fsWrite.Write(enbyte, 0, enbyte.Length);
                            }
                            else
                            {
                                byte[] enbyte = AesUtil.Encrypt(buffer, fmtPwd);
                                fsWrite.Write(enbyte, 0, enbyte.Length);
                            }
        ...
        public static int Decrypt(string srcFile, string dstFile, string password, bool overwrite = true)
        {
                    ...
                    using (FileStream fsWrite = new FileStream(dstFile, FileMode.Create))
                    {
                        ...
                        while ((readCount = fsRead.Read(buffer, 0, buffer.Length)) > 0)
                        {
                            if (readCount != buffer.Length)
                            {
                                byte[] temp = new byte[readCount];
                                Buffer.BlockCopy(buffer, 0, temp, 0, readCount);
                                byte[] debyte = AesUtil.Decrypt(temp, fmtPwd);
                                fsWrite.Write(debyte, 0, debyte.Length);
                            }
                            else
                            {
                                byte[] debyte = AesUtil.Decrypt(buffer, fmtPwd);
                                fsWrite.Write(debyte, 0, debyte.Length);
                            }
                        }
                    }
            ...
            string md5 = FileUtil.GetMD5(dstFile);
