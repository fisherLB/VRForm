...
namespace GraphicsEvaluatePlatform.Infrastructure.Common.Ftp
{
    public class FtpUtil
    {
        ...
        public FtpUtil(string FtpServerIP, string FtpRemotePath, string FtpUserID, string FtpPassword, string enc)
        {
        ...
        /// </summary> 
        /// <param name="filename"></param> 
        public void Upload(string filename, string folder, string targetName)
        {
            FileInfo fileInf = new FileInfo(filename);
            string uri = ftpURI + folder + "/" + targetName;
            FtpWebRequest reqFTP;
        ...
        /// </summary> 
        /// <param name="filename"></param> 
        public void UploadNoEx(string filename, string folder, string targetName)
        {
            FileInfo fileInf = new FileInfo(filename);
            string uri = ftpURI + folder + "/" + targetName;
            FtpWebRequest reqFTP;
        ...
        /// 下载 
        /// </summary> 
        /// <param name="soureFilePath">源ftp路径</param> 
        /// <param name="targetFileName">目标路径</param> 
        public void Download(string soureFilePath, string targetPath)
        {
            FtpWebRequest reqFTP;
            try
            {
                ...
                string filename = targetPath + "\\" + Path.GetFileName(soureFilePath);
                FileStream outputStream = new FileStream(filename, FileMode.Create);

                reqFTP = (FtpWebRequest)FtpWebRequest.Create(new Uri(ftpURI + soureFilePath));
        ...
        public void RemoveDirectorys(string folderName)
        {
            string curPath = ftpRemotePath + "/" + folderName;
            GotoDirectory(curPath, true);
            string[] folders = GetDirectoryList();
            if (folders == null)
                return;
            ...
            string parentFolder = curPath.Remove(curPath.LastIndexOf('/'));
            string orderFolder = curPath.Substring(curPath.LastIndexOf('/') + 1);
            GotoDirectory(parentFolder, true);
            RemoveDirectory(orderFolder);
        ...
        /// </summary> 
        /// <returns></returns> 
        public string[] GetAllFileList(string folder)
        {
        ...
        {
            string[] fileList = GetAllFileList(folder);
            string fname = Path.GetFileNameWithoutExtension(filename);
            string extension = Path.GetExtension(filename);
            string fnameExtension = Path.GetFileName(filename);
            string path = Path.GetDirectoryName(filename);

            bool isExist = FileExist(fileList, fnameExtension);
            int index = 1;
            while (isExist)
            {
                fname = fname + index.ToString();
                isExist = FileExist(fileList, fname + extension);
                index++;
            }
        ...
        private bool FileExist(string[] fileList, string RemoteFileName)
        {
            if (fileList == null)
                return false;
            foreach (string str in fileList)
            {
                if (str.ToLower().Trim() == RemoteFileName.ToLower().Trim())
                {
                    return true;
                }
            }
        ...
        /// </summary> 
        /// <returns></returns> 
        public string[] GetFileList(string mask, string folder)
        {
            string[] downloadFiles;
            StringBuilder result = new StringBuilder();
            string uri = ftpURI + folder;
        ...
        {
            Encoding defaultc = Encoding.GetEncoding("gb2312");
            Encoding utf = Encoding.UTF8;

            byte[] unicodeBytes = defaultc.GetBytes(s);

            string x = utf.GetString(unicodeBytes);
        ...
        public bool FileExist(string RemoteFileName, string folder)
        {
            string[] fileList = GetFileList("*.*", folder);
            foreach (string str in fileList)
            {
        ...
        public void MakeDirBat(string dirName)
        {
        ...
        public static void DeleteOrderDirectory(string ftpServerIP, string folderToDelete, string ftpUserID, string ftpPassword)
        {
            try
            {
                ...
                {
                    FtpUtil fw = new FtpUtil(ftpServerIP, folderToDelete, ftpUserID, ftpPassword);
                    //进入订单目录 
