...
namespace GraphicsEvaluatePlatform.Infrastructure
{
    ...
    public static class PublicHelper
    {
        ...
        public static void CheckArgument(object arg, string argName, bool canZero = false, bool stringCanNothing = true)
        {
            if (arg == null && stringCanNothing)
            {
                return;
            }
            else if (arg == null)
            ...
            {
                bool flag = stringCanNothing ? arg.CastTo<string>().Length >= 0 : arg.CastTo<string>().Length > 0;
                if (flag)
                {
