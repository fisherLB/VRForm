        ...
        public static OperationResult LoginValidate(string LoginName, string Password)
        {
            ...
            try
            {
                using (var tran = new TransactionScope(TransactionScopeOption.Required, new TransactionOptions() { IsolationLevel = System.Transactions.IsolationLevel.ReadUncommitted }))
                {
                    ...
                    //根据传入的帐号判断系统是否存在该帐号
                    dicCondition.Add("Name", LoginName);
                    ...
                    //为更新登录情况信息设置查询条件
                    dicCondition.Add("Password", dr["Password"]);
                    //检查密码是否正确
                    if (dr["Password"].ToString().Trim() != pwd)
                    {
                        return new OperationResult(OperationResultType.Error, "密码错误！");
                    }

                    //存放用户信息
                    BaseClass.USERID = retData.Tables[0].Rows[0]["Id"].ToString();  //登录用户ID
                    BaseClass.USERACCOUNTNAME = LoginName;  //存起登录用户名
                    BaseClass.VRTIME = retData.Tables[0].Rows[0]["VRTIME"].ToString();
                    BaseClass.USERNAME = retData.Tables[0].Rows[0]["Name"].ToString();
                    ...
                    ret.Message = "登录成功";
                    //UserlogBll.InsertLog(dr["Name"].ToString().Trim(), dr["Us_name"].ToString().Trim(), BaseClass.IPADDRESS, DateTime.Now, "系统登录", "登录成功 ", "");
